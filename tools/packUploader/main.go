...
func download() {
	...
	resp := postReq("pack/list", bodyJs)

	//
	var packs []Pack
	err = json.Unmarshal(resp, &packs)
	checkErr(err)
	...
	for _, pack := range packs {
		glog.Info("begin pack: ", pack.Title)
		dir := fmt.Sprintf("%04d%s", pack.Id, pack.Title)
		...
		// 	glog.Info("download image: ", image.File)
		// 	url := fmt.Sprintf("%s/%s", HOST, image.Key)
		// 	path := fmt.Sprintf("%s/%s", dir, image.File)

		// 	downloadFile(url, path)
		// }

		// //thumb
		// glog.Info("download thumb")
		// url := fmt.Sprintf("%s/%s", HOST, pack.Thumb)
		// ext := url[len(url)-3:]
		// path = fmt.Sprintf("%s/thumb.%s", dir, ext)
		// downloadFile(url, path)

		// //cover
		// glog.Info("download cover")
		// url = fmt.Sprintf("%s/%s", HOST, pack.Cover)
		// ext = url[len(url)-3:]
		// path = fmt.Sprintf("%s/cover.%s", dir, ext)
		// downloadFile(url, path)

		// //blur
		// glog.Info("download cover blur")
		// url = fmt.Sprintf("%s/%s", HOST, pack.CoverBlur)
		// ext = url[len(url)-3:]
		// path = fmt.Sprintf("%s/coverBlur.%s", dir, ext)
		// downloadFile(url, path)

		//pack.js
		glog.Info("write pack.js")
		path = fmt.Sprintf("%s/%s", dir, "pack.js")
