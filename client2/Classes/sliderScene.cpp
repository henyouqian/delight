...
bool SliderLayer::init(PackInfo *packInfo, EventInfo *eventInfo, PlayTicket *ticket) {
    ...
    _btnNext->addTargetWithActionForControlEvents(this, cccontrol_selector(SliderLayer::next), Control::EventType::TOUCH_UP_INSIDE);
    ...
    _btnBack->addTargetWithActionForControlEvents(this, cccontrol_selector(SliderLayer::ShowBackConfirm), Control::EventType::TOUCH_UP_INSIDE);
    ...
    _btnYes->addTargetWithActionForControlEvents(this, cccontrol_selector(SliderLayer::back), Control::EventType::TOUCH_UP_INSIDE);
    ...
    _btnNo->addTargetWithActionForControlEvents(this, cccontrol_selector(SliderLayer::HideBackConfirm), Control::EventType::TOUCH_UP_INSIDE);
    _btnNo->setOpacity(0);
    ...
    _btnFinish->addTargetWithActionForControlEvents(this, cccontrol_selector(SliderLayer::back), Control::EventType::TOUCH_UP_INSIDE);
    ...
    _btnNextPack->addTargetWithActionForControlEvents(this, cccontrol_selector(SliderLayer::nextPack), Control::EventType::TOUCH_UP_INSIDE);
...
void SliderLayer::onEnter() {
    ...
    _touchListener->onTouchBegan = CC_CALLBACK_2(SliderLayer::onTouchBegan, this);
...
void SliderLayer::ShowBackConfirm(Ref *sender, Control::EventType controlEvent) {
    //yes button
    auto fadeto = FadeTo::create(BTN_EASE_DUR, BTN_BG_OPACITY);
    auto easeFadeto = EaseSineOut::create(fadeto);
...
void SliderLayer::HideBackConfirm(Ref *sender, Control::EventType controlEvent) {
    //yes button
...
void SliderLayer::back(Ref *sender, Control::EventType controlEvent) {
    Director::getInstance()->popSceneWithTransition<TransitionFade>((Scene*)this->getParent(), .5f);
}

void SliderLayer::next(Ref *sender, Control::EventType controlEvent) {
...
void SliderLayer::nextPack(Ref *sender, Control::EventType controlEvent) {
    Director::getInstance()->popSceneWithTransition<TransitionFade>((Scene*)this->getParent(), .5f);
}

bool SliderLayer::onTouchBegan(Touch* touch, Event* event) {
    auto b = _gameplay->onTouchBegan(touch, event);
...
void SliderLayer::onTouchMoved(Touch* touch, Event* event) {
    _gameplay->onTouchMoved(touch, event);
}
void SliderLayer::onTouchEnded(Touch* touch, Event* event) {
    auto isComp = _gameplay->isCompleted();
    _gameplay->onTouchEnded(touch, event);
...
void SliderLayer::reset(int imgIdx) {
    _imgIdx = imgIdx;
    std::string local;
    ...
    //
    Sprite *dot = (Sprite*)(_dotBatch->getChildren().at(imgIdx));
    auto fadeTo = FadeTo::create(.3f, 255);
    auto ease = EaseSineOut::create(fadeTo);
    dot->runAction(ease);
    if (imgIdx > 0) {
        Sprite *prevDot = (Sprite*)(_dotBatch->getChildren().at(imgIdx-1));
